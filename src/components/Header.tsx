...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-card/20 backdrop-blur-sm shadow-lg shadow-background/30">
      <div className="container mx-auto flex h-20 items-center justify-center px-4 md:px-6">
        <nav className="hidden items-center gap-6 md:flex">
          <div className="rounded-full border border-border/50 bg-card/50 p-px backdrop-blur-sm">
            <div className="flex items-center gap-2 rounded-full bg-transparent px-4 py-2">
              {navLinks.map((link) => (
                <Link key={link.href} href={link.href} className="text-sm font-medium text-foreground/90 transition-colors hover:text-primary px-3 py-1.5 rounded-full">
                  {link.label}
                </Link>
              ))}
            </div>
